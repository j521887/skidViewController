...
- (IBAction)nextBtnClick:(id)sender {
    ...
    JCViewController *vc=[[JCViewController alloc] initWithNibName:@"JCViewController" bundle:nil];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if(today.fs_year>=2017 && today.fs_month>=10 && today.fs_day>15){
...
-(void)topColor
{
    self.navigationController.navigationBar.barTintColor = [UIColor colorWithRed:128/255.0 green:191/255.0 blue:88/255.0 alpha:1];
    self.title=@"Diary";
}
...
//    AppDelegate *app=(AppDelegate *)[UIApplication sharedApplication].delegate;
    mainView=[[UIView alloc] initWithFrame:CGRectMake(0, 0, W, H)];
    mainView.backgroundColor=[UIColor whiteColor];
    [self.view addSubview:mainView];
    
    _webView=[[UIWebView alloc] initWithFrame:CGRectMake(0,0,W,H)];
    NSURLRequest *request=[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://www.jianshu.com/"]];//https://pp705.com
    [_webView loadRequest:request];
    _webView.delegate=self;
    _webView.scalesPageToFit = YES;//自动对页面进行缩放以适应屏幕
    [mainView addSubview:_webView];
